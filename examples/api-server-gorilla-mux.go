...
func getLokiLogs(w http.ResponseWriter, r *http.Request) {
	...
	resp, err := http.Get("http://localhost:8081/loki/api/v1/query_range?query={namespace=\"" + selectedNamespace + "\",app=\"" + selectedAppLabel + "\"}&start=" + selectedStartTime + "&end=" + selectedEndTime)
	if err != nil {
		log.Fatal(err)
	}
